...
Polygon.prototype = {
    ...
    getHorizontalRectangles: function () {
		if (!this.hRectangles) {
			this.hRectangles = [];    
			var slices = this._createSlicesWithEdges();
    ...
    //returns an array of y indexes of the horizontal slices
    _getHorizontalSlices: function () {
        var slices = [];
        var points = this.points;
        
        for (var i = 0; i < points.length; i++) {
            if (slices.indexOf(points[i].y) == -1) {
                slices.push(points[i].y);
            }
        }
    ...
    //creates an array of slices. Each slice contains a list of horizontal edges positioned on the slice's y axis.
    _createSlicesWithEdges: function () {
        var res = [];
        var points = this.points;
        ...
        for (var i = 0; i < points.length; i++) {        
            ...
            if (p1_y == p2_y) { //taking only orizontal edges        
                ...
                if (!currSlice) {
                    currSlice = new Slice(points[i].y);
                    res.push(currSlice);
                }
                
                if (p2_x < p1_x) { //sort points from left to right on x-axis
                    var temp = p1_x;
                    p1_x = p2_x;
                    p2_x = temp;
                }
                
                currSlice.addEdge(p1_x, p2_x);
            }
        }
            ...
            for (var slice_edge in edges) {
                ...
                if (slice_edge.p1.x <= left_x && slice_edge.p2.x > left_x) {
                    ...
                    var pol = new Polygon([
                        ...
                        new Point(right_x, slice.y),
                        new Point(left_x, slice.y)
                    ]);
